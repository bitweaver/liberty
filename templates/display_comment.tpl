...
<div style="margin-left: {math equation="level * marginIncrement" level=$comment.level marginIncrement=20}px">
	<div class="post">
		...
		<div class="footer">
			{if $gBitUser->hasPermission( 'bit_p_post_comment' )}<a href="{$comments_return_url}&amp;post_comment_reply_id={$comment.content_id}&amp;post_comment_request=1" rel="nofollow">{biticon ipackage="liberty" iname="reply" iexplain="Reply to this comment"}</a>{/if}
