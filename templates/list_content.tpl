...
<div class="listing liberty">
	<header>
		{form id="contentlist" class="form-search pull-right" action=$smarty.server.REQUEST_URI method="get"}
			...
			{/if}
				<input type="text" name="find" value="{$listInfo.find}" class="input-xs search-query" placeholder="{tr}Search Within Results{/tr}">
				<input type="submit" class="btn btn-default btn-xs" value="{tr}Search{/tr}" name="search_objects"/>
			</div>
		{/form}
		<h1>
