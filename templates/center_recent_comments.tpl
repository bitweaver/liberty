...
<div class="">
	...
	<div class="body comments">
		...
		{section name=ix loop=$modLastComments}
			<li class="item">
				<a rel="ugc" href="{$modLastComments[ix].display_url}"><img style="width:50px;float:left;" src="{$modLastComments[ix].object->getThumbnailUrl($moduleParams.module_params.thumb_size)|default:"`$smarty.const.USERS_PKG_URL`icons/silhouette.png"}" alt="{$modLastComments[ix].object->getTitle()|escape}" title="{$modLastComments[ix].object->getTitle()|escape}" /></a>
...
<div>- {displayname hash=$modLastComments[ix]}</div>
					<div class="actiontext"><a rel="ugc" href="{$modLastComments[ix].display_url}">{tr}View{/tr}</a>{* <a rel="ugc" href="{$modLastComments[ix].display_url}{if strpos($modLastComments[ix].display_url,'?')}&amp;{else}?{/if}post_comment_reply_id={$modLastComments[ix].parent_id}&amp;post_comment_request=1">{tr}Reply{/tr}<a/>*}</div>
