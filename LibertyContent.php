...
*
* @package  liberty
* @version  $Header: /cvsroot/bitweaver/_bit_liberty/LibertyContent.php,v 1.2.2.20 2005/08/07 13:19:07 lsces Exp $
...
//				$this->mInfo['perm_level'] = $this->getUserPermissions();
	...
	function store( &$pParamHash ) {
		...
		if( LibertyContent::verify( $pParamHash ) ) {
			...
			if( empty( $pParamHash['content_id'] ) ) {
				$pParamHash['content_store']['content_id'] = $this->getDb()->GenID( 'tiki_content_id_seq' );
				$pParamHash['content_id'] = $pParamHash['content_store']['content_id'];
				$result = $this->getDb()->associateInsert( $table, $pParamHash['content_store'] );
			} else {
				...
				$result = $this->getDb()->associateUpdate( $table, $pParamHash['content_store'], $locId );
			}
...
*/
	...
	function loadPermissions() {
		if( $this->isValid() && empty( $this->mPerms ) && $this->mContentTypeGuid ) {
			...
			$this->mPerms = $this->getDb()->getAssoc($query, $bindVars);
		}
    ...
    */
	...
	function getUserPermissions( $pUserId ) {
		...
		if( $pUserId ) {
			...
			$ret = $this->getDb()->getAssoc($query, $bindVars);
		}
		return $ret;
	}
	...
	function storePermission( $pGroupId, $perm_name, $object_id=NULL ) {
		...
		$query = "insert into `".BIT_DB_PREFIX."users_objectpermissions`
				  ...
				  VALUES ( ?, ?, ?, ? )";
		$result = $this->getDb()->query($query, array($pGroupId, $object_id, $this->mContentTypeGuid, $perm_name));
		return true;
	}
	...
	**/
	function getAuthorList( &$pListHash ) {
		...
		if( $result = $this->getDb()->query( $query, $bindVars, $pListHash['max_records'], $pListHash['offset'] ) ) {
			$ret = $result->GetRows();
		}
		return $ret;
	}
...
// 					$gateFrom .= " LEFT OUTER JOIN  `".BIT_DB_PREFIX."tiki_fisheye_gallery_image_map` tfgim ON (tfgim.`item_content_id`=tc.`content_id`)";
		...
		$query = "SELECT uue.`login` AS `modifier_user`, uue.`real_name` AS `modifier_real_name`, uue.`user_id` AS `modifier_user_id`, uuc.`login` AS`creator_user`, uuc.`real_name` AS `creator_real_name`, uuc.`user_id` AS `creator_user_id`, `hits`, tc.`title`, tc.`last_modified`, tc.`content_type_guid`, `ip`, tc.`content_id` $gateSelect
				  ...
				  ORDER BY ".$orderTable.$this->getDb()->convert_sortmode($sort_mode);
	...
	**/
	function getStructures() {
		...
		if( $this->isValid() ) {
			...
			if( $result = $this->getDb()->query( $query,array( $this->mContentId ) ) ) {
				while ($res = $result->fetchRow()) {
					$ret[] = $res;
					$result->MoveNext();
				}
	...
	function isCached($url) {
		...
		$ret = $this->getDb()->getOne($query, array( $url ) );
		return( isset( $ret ) );
	}
	...
	function cacheUrl($url, $data = '') {
		...
		if (!$this->isCached( $url ) && $data)
		{
			...
			$result = $this->getDb()->query($query, array($url,BitDb::db_byte_encode($data),$refresh) );
			return !isset($error);
		}
		else return false;
	}
	...
	function isInStructure( $pStructureId=NULL ) {
		if( $this->isValid() ) {
			...
			$cant = $this->getDb()->getOne( $query, $bindVars );
			return $cant;
		}
	}
